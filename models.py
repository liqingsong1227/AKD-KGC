...
class Model(nn.Module):
    ...
    def __init__(self, belman_model_cfg, data, lm_model): #, embedding_path
        # kept that because super Ultra sounds cool
        super(Model, self).__init__()
        self.belman_model = NBFNet(**belman_model_cfg)
        self.lmke_model = LMKE(lm_model, data)
        input_dim = belman_model_cfg['input_dim']
        hidden_dims = belman_model_cfg['hidden_dims']
        language_dim = lm_model.config.hidden_size
        feature_dim = (sum(hidden_dims) if self.belman_model.concat_hidden else hidden_dims[-1]) + input_dim + language_dim * 4 + 2
