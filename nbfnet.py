...
class NBFNet(nn.Module):
    ...
    def remove_easy_edges(self, graph, h_index, t_index, r_index=None):
        ...
        # if batch_neg do not add reverese edge
        # h_index_ext = h_index
        # t_index_ext = t_index
        # r_index_ext = r_index
        if self.remove_one_hop:
            ...
            mask = ~index_to_mask(index, graph.num_edges)
        else:
            # we remove existing immediate edges between heads and tails in the batch with the given relation
            edge_index = torch.cat([graph.edge_index, graph.edge_type.unsqueeze(0)])
    ...
    def bellmanford(self, graph, h_index, r_index, separate_grad=False):
        ...
        query = self.query(r_index)  # batch * dim
       
        index = h_index.unsqueeze(-1).expand_as(query)

        # initial (boundary) condition - initialize all node states as zeros
        boundary = torch.zeros(batch_size, graph.num_nodes, self.dims[0], device=h_index.device)
        # by the scatter operation we put query (relation) embeddings as init features of source (index) nodes
        boundary.scatter_add_(1, index.unsqueeze(1), query.unsqueeze(1))
        size = (graph.num_nodes, graph.num_nodes)
        edge_weight = torch.ones(graph.num_edges, device=h_index.device)
        ...
        for layer in self.layers:
            ...
            hidden = layer(layer_input, query, boundary, graph.edge_index, graph.edge_type, size, edge_weight)
        ...
        node_query = query.unsqueeze(1).expand(-1, graph.num_nodes, -1) # (batch_size, num_nodes, input_dim)
    ...
    def forward(self, graph, batch, training=True):
        ...
        h_index, t_index, r_index = tasks.negative_sample_to_tail(graph, h_index, t_index, r_index)

        assert (h_index[:, [0]] == h_index).all()
        assert (r_index[:, [0]] == r_index).all()

        # message passing and updated node representations
        output = self.bellmanford(graph, h_index[:, 0], r_index[:, 0])  # (num_nodes, batch_size, feature_dim）
        feature = output["node_feature"]
        index = t_index.unsqueeze(-1).expand(-1, -1, feature.shape[-1]) # batch * n_neg+1 * dim
    ...
    def forward_no_score(self, graph, batch, training=True):
        h_index, t_index, r_index = batch.unbind(-1)
        if training:
            # Edge dropout in the training mode
            # here we want to remove immediate edges (head, relation, tail) from the edge_index and edge_types
    ...
    def visualize(self, graph, batch):
        ...
        distances, back_edges = self.beam_search_distance(graph, edge_grads, h_index, t_index, self.num_beam)
        paths, weights = self.topk_average_length(distances, back_edges, t_index, self.path_topk)

        return paths, weights

    @torch.no_grad()
    def beam_search_distance(self, graph, edge_grads, h_index, t_index, num_beam=10):
        ...
        input[h_index, 0] = 0
        edge_mask = graph.edge_index[0, :] != t_index

        distances = []
        back_edges = []
        for edge_grad in edge_grads:
            # we don't allow any path goes out of t once it arrives at t
            node_in, node_out = graph.edge_index[:, edge_mask]
            relation = graph.edge_type[edge_mask]
