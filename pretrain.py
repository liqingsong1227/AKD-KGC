...
if __name__ == '__main__':
	...
	parser.add_argument('--nbf_lr', type=float, default=2e-3)
	...
	data_paths = {'fb15k-237':{
			'dataset': 'fb15k-237',
			'train': './data/pretrain/fb15k-237/train.tsv',
			'valid': './data/pretrain/fb15k-237/dev.tsv',
			'test': './data/pretrain/fb15k-237/test.tsv',
			'description': './data/pretrain/fb15k-237/entity2description.txt',
			'name':['./data/pretrain/fb15k-237/entity2name.txt', './data/pretrain/fb15k-237/relation2text.txt'],
			'entitydict':'./data/pretrain/fb15k-237/entities.dict',
			'relationdict':'./data/pretrain/fb15k-237/relations.dict'
		},
		'wn18rr':{
			'dataset': 'wn18rr',
			'train': './data/pretrain/WN18RR/train.tsv',
			'valid': './data/pretrain/WN18RR/dev.tsv',
			'test': './data/pretrain/WN18RR/test.tsv',
			'description': './data/pretrain/WN18RR/entity2description.txt',
			'name':['./data/pretrain/WN18RR/entity2name.txt', './data/pretrain/WN18RR/relation2text.txt'],
			'entitydict': './data/pretrain/WN18RR/entities.dict',
			'relationdict': './data/pretrain/WN18RR/relations.dict'
		},
		'codex-m': {
			'dataset': 'codex-m',
			'train': './data/pretrain/codex/codex-m/train.tsv',
			'valid': './data/pretrain/codex/codex-m/valid.tsv',
			'test': './data/pretrain/codex/codex-m/test.tsv',
			'description': './data/pretrain/codex-m/entity2description.txt',
			'name':['./data/pretrain/codex-m/entity2name.txt', './data//pretrain/codex-m/relation2text.txt'],
			'entitydict': './data/pretrain/codex-m/entities.dict',
			'relationdict': './data/pretrain/codex-m/relations.dict'
		}
	}
	...
	in_path = data_paths[dataset_name]
	data = DataProcess(in_path, lm_tokenizer, lm_model, max_desc_length = arg.max_desc_length, add_tokens = arg.add_tokens, 
	...
	train_dataset = torch.cat([data.train_data.target_edge_index, data.train_data.target_edge_type.unsqueeze(0)]).t()
	valid_dataset = torch.cat([data.valid_data.target_edge_index, data.valid_data.target_edge_type.unsqueeze(0)]).t()
	test_dataset = torch.cat([data.test_data.target_edge_index, data.test_data.target_edge_type.unsqueeze(0)]).t()

	train_graph = data.train_data
	...
	identifier = 'nbf-{}-{}-batch_size={}-max_desc_length={}-data-{}'.format(arg.plm, arg.batch_size, arg.max_desc_length, dataset_name)
	
	if arg.data == 'fb15k-237':
		nbf_model_cfg = {'input_dim': 32,'hidden_dims': [32, 32, 32, 32, 32, 32],'message_func': 'distmult', 'aggregate_func': 'pna',
						'short_cut': True, 'layer_norm': True, 'num_relation':data.train_data.num_relations, 'remove_one_hop':True, 'dependent':True}
	elif arg.data == 'wn18rr':
		nbf_model_cfg = {'input_dim': 32,'hidden_dims': [32, 32, 32, 32, 32, 32],'message_func': 'distmult', 'aggregate_func': 'pna',
						'short_cut': True, 'layer_norm': True, 'num_relation':data.train_data.num_relations, 'remove_one_hop':False, 'dependent':True}
	elif arg.data == 'codex-m':
		nbf_model_cfg = {'input_dim': 32,'hidden_dims': [32, 32, 32, 32, 32, 32],'message_func': 'distmult', 'aggregate_func': 'pna',
						'short_cut': True, 'layer_norm': True, 'num_relation':data.train_data.num_relations, 'remove_one_hop':False, 'dependent':True}

	ultra = NBFNet(**nbf_model_cfg)


	
	
	nbf_param = [	
		{'lr': arg.nbf_lr, 'params' : [p for n, p in ultra.named_parameters()   #.belman_model
									if (not any(nd in n for nd in no_decay))],  
		'weight_decay': arg.weight_decay},
		{'lr': arg.nbf_lr, 'params': [p for n, p in ultra.named_parameters()
									if (any(nd in n for nd in no_decay))],
		'weight_decay': 0.0}
	]
	nbf_param_name = list(set(['.'.join(n.split('.')[:2]) for n, p in ultra.named_parameters()]))

	
	param_group =  nbf_param
	total_param_name =  nbf_param_name
	...
	trainer = Trainer(data, data_loader_list, ultra, optimizer, scheduler, device=device, hyperparams=hyperparams) #  
	if arg.finetune:
		trainer.run()
