...
class LMKE(nn.Module):
	def __init__(self, lm_model, data):
		super().__init__()
		...
		self.ent_embeddings = torch.nn.Embedding(data.entity_num, self.hidden_size)
		self.rel_embeddings = torch.nn.Embedding(data.relation_num , self.hidden_size) # * 2
		# self.ent_embedding_final = torch.nn.Embedding(data.entity_num, self.hidden_size)
		# self.ent_embedding_final.weight.data.copy_(data.entity_embedding)
		# self.ent_embedding_final.weight.requires_grad = True
		...
		self.data = data
		degrees = data.statistics['degrees']
		self.degrees =  torch.Tensor([degrees.get(i, 0) for i in range(data.entity_num)])  
	...
	def forward(self, graph, batch, hr_inputs, hr_positions, t_inputs, t_positions, mode):
		...
		degrees = self.degrees.unsqueeze(0).expand(h_index.shape[0], -1).to(h_index.device) # batch * n_ent

		h_deg= degrees.gather(1,h_index)
		t_deg = degrees.gather(1,t_index) # batch * (1+n_neg)
		h_logdeg,  t_logdeg = (h_deg+1).log(), (t_deg+1).log()
		deg_feature = torch.stack([h_logdeg, t_logdeg], dim=2)   # batch * (1+n_neg) * 2
		...
		# total_feature = torch.cat([merge_feature, deg_feature],dim=-1)
		total_feature = torch.cat([hr_preds, t_preds, hr_preds-t_preds, hr_preds*t_preds, deg_feature], dim=-1)  # mlp for wn18rr
	...
	def forward_test(self, graph, batch, hr_inputs, hr_positions, t_preds, mode):
		...
		deg_feature = torch.stack([h_logdeg, t_logdeg], dim=2).to(hr_preds.device)
